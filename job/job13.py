...
# result7 = g.geocode('上河檀府')
# result8 = CoordinatesConverter.gcj02towgs84(result7[0], result7[1])
# geometry = [Point((x,y)) for x,y in zip(df_points["经度"], df_points["纬度"])]
geometry = []
lng_list = []
lat_list = []

for name in df_points["项目名称"]:
    time.sleep(0.5)
    name1 = "宁波市"+name
    print(name1)
    coords = g.geocode(name1)



    if coords:  # 若 geocode 成功
        result = CoordinatesConverter.gcj02towgs84(coords[0], coords[1])
        print(result)
        lng, lat = result
        lng_list.append(lng)
        lat_list.append(lat)
        geometry.append(Point(result))
    else:  # 若失败，可用 None 或略过
        geometry.append(None)
        lng_list.append(None)
        lat_list.append(None)

df_points["经度"] = lng_list
df_points["纬度"] = lat_list
...
# === 2. 读取所有面（GeoJSON 文件） ===
polygons = gpd.read_file("all_surfaces0.geojson").to_crs("EPSG:4326")
...
# 例如你想保留的字段
poly_fields = ['assignmentArea', 'clinchConfirmTime', 'bidWay', 'dealPrice','RJL_S','resName','resNumber']  # 可根据面字段选择
prefix = ""
