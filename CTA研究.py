...
def replay(files):
    ...
    with st.expander("设置回放参数", expanded=False):
        with st.form(key='my_form_replay'):
            ...
            max_k_num = int(col5.number_input(label='最大绘制K线数', value=1000, min_value=50, max_value=6000))
...
def backtest(files):
    ...
    with st.form(key='my_form_czsc'):
        ...
        max_workers = int(col5.number_input(label='指定进程数量', value=os.cpu_count() // 4,
                                            min_value=1, max_value=os.cpu_count() // 2))
